...
export default function useCollaborators(orgId: string | null) {
	...
	const { data: Organizations, status } = useAppSelector(
	...
	const updateCollaborator = async (
		...
		setLoadRol(id)
		const { data } = await CollaboratorServices.update(id, {
			rol: collaborator.rol,
		})
		if (data) {
			const newColla = collaborators.filter(
				collaborator => collaborator.id !== id
			)
			if (myUser) {
				const newStateOrganization = Organizations.filter(
					org => org.Organization.id !== collaborator.Organization.id
				)
